...
parser.add_argument("-e", 			dest = "epochs", 		type = int, 	default = 15, 			help = "Number of Training Epochs (Default: 25)")
...
parser.add_argument("-h1", 		dest = "h1", 			type = int, 	default = 300, 	help = "Dimensionality of the Aspect-level Representations (Default: 10)")
...
# Loading aspect embeddings and aspect list
aspect_embeddings_path = "./datasets/{}/{}_aspect_embeddings.pkl".format(args.dataset, args.dataset)
with open(aspect_embeddings_path, 'rb') as f:
    aspect_embeddings = pickle.load(f)

# 设置args.aspect_list为所有方面的名称
args.aspect_list = list(aspect_embeddings.keys())

# 你可以查看加载的方面
# print("Loaded aspects:", args.aspect_list)

# 使用这些方面嵌入
args.aspect_embeddings = aspect_embeddings
...
#		all_rating_pred.extend(rating_pred.data)
		all_rating_true.extend(batch_rating.cpu().numpy())  # Ensure batch_rating is a NumPy array
		all_rating_pred.extend(rating_pred.cpu().detach().numpy())  # Convert to NumPy array and detach

	MSE = mean_squared_error(all_rating_true, all_rating_pred)
	MAE = mean_absolute_error(all_rating_true, all_rating_pred)
	RMSE = np.sqrt(MSE)
	logger.log("[{}] {:6s} MSE: {:.5f}, RMSE: {:.5f}, MAE: {:.5f}".format( "Epoch {:d}".format( epoch_num + 1 ) if epoch_num >= 0 else "Initial",
		"[{}]".format( phase ), MSE, RMSE, MAE), print_txt = print_txt)
...
#		losses.append(loss.data[0])
	...
	devMSE, _ , _= evaluate(mdl, dev_loader, epoch_num = epoch_num, use_cuda = args.use_cuda, phase = "Dev")
	testMSE, testRMSE ,testMAE = evaluate(mdl, test_loader, epoch_num = epoch_num, use_cuda = args.use_cuda, phase = "Test")
...
epoch_num_forBestDevMSE, bestDevMSE, testMSE_forBestDevMSE, testRMSE_forBestDevMSE,testMAE_forBestDevMSE = getBestPerf(lstDevMSE, lstTestMSE, lstTestRMSE,lstTestMAE)
logger.log("\nBest Dev MSE: {:.5f} (Obtained during Evaluation #{:d})".format( bestDevMSE, epoch_num_forBestDevMSE ))
logger.log("Test MSE: {:.5f},Test RMSE: {:.5f}, Test MAE: {:.5f}".format( testMSE_forBestDevMSE,testRMSE_forBestDevMSE ,testMAE_forBestDevMSE ))
